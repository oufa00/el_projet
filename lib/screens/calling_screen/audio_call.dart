...
class _AudioCallState extends State<AudioCall> {
  ...
  Future<void> initialize() async {
    String Agora_APP_IDD = '5e63c7885f4f4b149d741060375593f4';
    ...
    _engine.setEventHandler(RtcEngineEventHandler(error: (code) {
      ...
      if (widget.currentuseruid == widget.call.callerId) {
        ...
        FirebaseFirestore.instance
            ...
            .doc(widget.call.receiverId)
            .set({
          'audioCallRecieved': FieldValue.increment(1),
        }, SetOptions(merge: true));
        FirebaseFirestore.instance.collection('dashboard').doc('chatdata').set({
          'audiocallsmade': FieldValue.increment(1),
        }, SetOptions(merge: true));
      }
      Wakelock.enable();
        ...
        children: [
          Text(
            ...
            style: TextStyle(
              ...
              color: status == 'pickedup' ? Colors.green : Colors.white,
              fontSize: 25,
            ),
          ),
          Padding(
            ...
            child: Text(
              status == 'pickedup' ? 'picked' : 'voice',
              style: TextStyle(
                fontWeight: FontWeight.normal,
                color: status == 'pickedup' ? Colors.lightGreen : Colors.green,
                fontSize: 16,
              ),
            ),
          ),
              ...
              body: StreamBuilder<DocumentSnapshot<Map<String, dynamic>>>(
                ...
                builder: (BuildContext context, snapshot) {
                  if (snapshot.hasData) {
                    if (snapshot.data!.data() == null ||
                        snapshot.data == null) {
                      print(snapshot.data!.data()!["STATUS"]);
                      print("3afssa");
                      return Center(
                        child: Stack(
                          children: <Widget>[
